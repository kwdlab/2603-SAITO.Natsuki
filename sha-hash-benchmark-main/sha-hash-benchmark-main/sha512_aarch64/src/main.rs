...
fn main() {
    ...
    println!();
    println!("実行時間: {:.9}秒", elapsed.as_secs_f64());
    println!();
    ...
    for i in 0..ITERATIONS {
        ...
        unsafe { sha512_compress_hw(black_box(&mut state), black_box(&data)); }
        let elapsed = start.elapsed();
        let time_ns = elapsed.as_nanos();
        let time_sec = elapsed.as_secs_f64();
        
        // OSのタイマー分解能により 0ns と測定された場合をカウント
        if time_ns == 0 {
            zero_count += 1;
        }
